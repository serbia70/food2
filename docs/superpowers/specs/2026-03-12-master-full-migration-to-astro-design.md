...
> 注：JSON 字段 key **大小写敏感**，以下 payload 字段名完全按旧页实际发送为准。
...
| POST /api/master/manage (action=update_rate_center) | action | 否 | “系统设置”Tab → “💾 保存汇率配置” | UI：`meituanGo/static/master.html:1051-1152`；请求：`meituanGo/static/master.html:2403-2411` | `base_rate`,`manual_offset`,`daily_step` | 保存成功并提示（旧页 `alert("✅ 汇率已保存")`） | ☐ | ☐ |
| POST /api/master/manage (action=get_shop_billing) | action | 否 | 店铺列表 → “编辑(✏️)”打开弹窗后自动加载余额 | UI：`meituanGo/static/master.html:2017-2022,2584-2587`；请求：`meituanGo/static/master.html:2470-2490` | `id` | 返回账单数据并回显到“当前余额”（旧页更新 `#edit-billing-balance`） | ☐ | ☐ |
| POST /api/master/manage (action=adjust_shop_balance) | action | 是 | 店铺编辑弹窗 → “💰 钱包充值”面板 → “充值”按钮 | UI：`meituanGo/static/master.html:1620-1701`；请求：`meituanGo/static/master.html:2503-2550` | RSD 充值：`id`,`amountRsd`,`entryType`,`note`；CNY 充值：`id`,`sourceCurrency`,`sourceAmount`,`entryType`,`note` | 余额变更并回显（旧页更新余额、清空输入并 `alert("✅ 充值成功")`） | ☐ | ☐ |
| POST /api/master/manage (action=update_shop) | action | 否 | 店铺编辑弹窗 → “💾 保存” | UI：`meituanGo/static/master.html:1466-1475,1724-1726`；请求：`meituanGo/static/master.html:2589-2622` | `id`,`name`,`slug`,`expireDate`,`lastPaidMonth`,`commissionMode`,`commissionType`,`commissionValue`,`enableDelivery`,`enableDineIn`,`enableReservation`,`newPassword` | 更新成功后提示并 `location.reload()` | ☐ | ☐ |
| POST /api/master/manage (action=set_shop_plan) | action | 否 | 店铺编辑弹窗保存时顺带设置套餐（非独立按钮） | 请求：`meituanGo/static/master.html:2614-2617` | `id`,`planType` | 更新套餐成功后继续完成保存流程（失败则中断） | ☐ | ☐ |
| POST /api/master/manage (action=create_shop) | action | 否 | “创建店铺”Tab → “✨ 创建店铺” | UI：`meituanGo/static/master.html:751-785`；请求：`meituanGo/static/master.html:2624-2637` | `name`,`phone`,`password` | 创建成功提示并刷新（旧页 `alert("✅ 创建成功: " + res.slug)` + reload） | ☐ | ☐ |
| POST /api/master/manage (action=delete_shop) | action | 是 | 店铺列表 → 删除按钮（🗑️） | UI：`meituanGo/static/master.html:2140-2169`；请求：`meituanGo/static/master.html:2640-2646` | `ID`（大小写敏感；旧页就是大写 `ID`） | 删除成功后刷新（旧页 `location.reload()`） | ☐ | ☐ |
| POST /api/master/manage (action=update_password) | action | 是 | “系统设置”Tab → “🔑 修改超级密码” → “💾 保存” | UI：`meituanGo/static/master.html:1212-1225`；请求：`meituanGo/static/master.html:2649-2661` | `newPassword` | 成功提示并强制重新登录（旧页清 token + reload） | ☐ | ☐ |
| POST /api/master/manage (action=trigger_backup) | action | 是 | “📦 数据备份与还原（zip）”区块 → “⚡ 立即备份并下载 (本地)” | UI：`meituanGo/static/master.html:1228-1250`；请求：`meituanGo/static/master.html:2664-2674` | （空对象） | 触发成功并回显 message（旧页 `alert(result.message)`） | ☐ | ☐ |
| POST /api/master/manage (action=batch_update_commission) | action | 是（批量） | 店铺管理 Tab → “🚀 执行批量替换” | UI：`meituanGo/static/master.html:643-688`；请求：`meituanGo/static/master.html:2709-2723` | `oldVal`,`newVal`,`type` | 批量更新并提示变更数量（旧页 `alert(...)` + reload） | ☐ | ☐ |
| POST /api/master/manage (action=approve_renew) | action | 否 | “待办审核”Tab → “✅ 确认” | UI：`meituanGo/static/master.html:741-748,1890-1954`；请求：`meituanGo/static/master.html:2726-2735` | `ID`（大小写敏感；旧页就是大写 `ID`） | 审批成功提示并刷新 | ☐ | ☐ |
| POST /api/master/manage (action=reject_renew) | action | 否 | “待办审核”Tab → “❌ 驳回” | UI：`meituanGo/static/master.html:741-748,1890-1954`；请求：`meituanGo/static/master.html:2738-2745` | `ID`（大小写敏感；旧页就是大写 `ID`） | 驳回成功并刷新 | ☐ | ☐ |
| POST /api/master/backup | endpoint | 是 | “系统设置”Tab → **代码备份（Code Backup）**区块（列表/创建/删除）；注意：不同于“📦 数据备份与还原(zip)”区块 | 列表：`meituanGo/static/master.html:2748-2801`；创建：`meituanGo/static/master.html:2804-2833`；删除：`meituanGo/static/master.html:2836-2846` | JSON：`action` ∈ `list`/`create`/`delete`；当 `action=list`：仅 `action`；当 `action=create`：`action`,`backupName`（必填）；当 `action=delete`：`action`,`backupName`（必填） | 列表刷新/创建后可见/删除后不可见 | ☐ | ☐ |
| POST /api/master/restore | endpoint | 是 | “📦 数据备份与还原（zip）”区块 → “🚀 开始还原”（选择 .zip 文件） | UI：`meituanGo/static/master.html:1261-1277`；请求：`meituanGo/static/master.html:2676-2706` | `multipart/form-data`：`file`（zip 文件） | 恢复并强提示风险；成功后提示并刷新 | ☐ | ☐ |
| POST /api/master/upload | endpoint | 否 | “系统设置”Tab → 各种“上传”按钮（二维码/收款码） | UI：`meituanGo/static/master.html:1033-1046,1162-1192`；请求：`meituanGo/static/master.html:2849-2893` | `multipart/form-data`：`file`（image/*） | 返回 `{ success, url }` 并回填到目标 input（旧页 `#<targetId>.value = data.url`） | ☐ | ☐ |
