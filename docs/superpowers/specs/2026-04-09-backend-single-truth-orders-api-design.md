...
- `foos2Go` 里还保留大量 `_test.go`，用户已明确要求最终全部移除。

用户已明确选定本轮方向：
- 选 **后端统一单一真源**；
- **数据库内部可以保留 snake_case**，但 **HTTP 对外一律 camelCase**；
- **删除 foos2Go 里的 `_test.go`**；
...
5. 删除 `foos2Go` 下全部 `_test.go`，最终仓库不再保留 Go 测试文件。
...
### 4.4 删除 foos2Go 全部 `_test.go`
用户已明确要求最终仓库不保留 `foos2Go` 的 `_test.go`。因此本轮最终状态应满足：
- `foos2Go/**/*.go` 中不存在 `_test.go`；
- 现有 `_test.go` 只可作为迁移时的临时参考；
- 最终提交内容必须删除全部 Go 测试文件。
...
## 12. 验证方式
由于用户要求最终删除 `foos2Go` 的 `_test.go`，本轮验收以**生产链路构建 + 正式接口烟雾验证 + 全文搜索**为主。
