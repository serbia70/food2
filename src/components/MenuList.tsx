          ...
          <div className={`menu-grid-adaptive ${isOneCol ? 'layout-1col' : 'layout-2col'}`}>
            {cat.products.map((p) => {
              ...
              const itemStyle = isTextMode ? {
                ...
                padding: isOneCol ? '10px 12px' : '8px 10px', // 两列模式下缩小边距
                minHeight: '60px',
                boxShadow: '0 1px 3px rgba(0,0,0,0.02)',
              ...
              return (
                <div key={p.id} style={itemStyle}>
                  {/* LEFT: IMAGE (Hidden in Text Mode) */}
                  {hasImg && !isTextMode && (
                    <div
                      style={{
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: isTextMode ? 'center' : 'space-between', minWidth: 0, paddingLeft: '4px' }}>
                    <div style={{ overflow: 'hidden' }}>
                        ...
                        <div style={{ fontSize: '11px', color: '#a0aec0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.sub_name}</div>
                    </div>
                    
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: isTextMode ? '4px' : 'auto', gap: '2px' }}>
                        <div style={{ color: '#e53e3e', fontWeight: '800', fontSize: isTextMode ? (isOneCol ? '16px' : '14px') : '14px', whiteSpace: 'nowrap', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {p.price} <small style={{fontSize:'9px', fontWeight:'normal', opacity:0.7}}>RSD</small>
                        </div>
