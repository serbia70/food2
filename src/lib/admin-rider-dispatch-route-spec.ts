...
test('publish dispatch /api/admin/orders 返回 HTTP 200 success:false 时返回 order_fetch_failed', async (t) => {
  useTestEnv(t);
  ...
  assert.deepEqual(body, {
    ...
    upstream_status: 200,
    upstream_body: '{"success":false,"error":"orders_payload_failed"}',
  });
  assert.equal(calls.some((call) => call.url.endsWith('/api/admin/orders/923/status')), false);
  assert.equal(calls.some((call) => call.url.endsWith('/api/admin/orders/remarks')), false);
  assert.equal(calls.some((call) => call.url.endsWith('/api/telegram/send')), false);
});

test('publish dispatch 可读取顶层 order 单对象包装并继续成功派单', async (t) => {
  useTestEnv(t);
  ...
  const body = JSON.parse(await response.text()) as {
    success?: boolean;
    telegram_dispatch?: { deliveredCount?: number; failedCount?: number };
  };
  const telegramCall = calls.find((call) => call.url.endsWith('/api/telegram/send'));
  ...
  assert.ok(telegramCall);
  assert.equal(calls.some((call) => call.url.endsWith('/api/admin/orders/924/status')), true);
});
