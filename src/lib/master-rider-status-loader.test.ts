...
test('loadMasterRiderStatusData uses POST impersonate and cookie auth for riders proxy', async (t) => {
  const calls = useMockFetch(t, async (request) => {
    ...
    if (url.pathname === '/api/master/impersonate-shop') {
  ...
  assert.deepEqual(calls.map((call) => `${call.method} ${new URL(call.url).pathname}`), [
    'POST /api/master/impersonate-shop',
...
test('loadMasterRiderStatusData keeps cross-shop dedupe with cookie-based impersonation', async (t) => {
  ...
  assert.deepEqual(
    result.payload.groups.available.map((rider) => rider.name),
    ['Rider A'],
  );
  assert.deepEqual(
    result.payload.groups.busy.map((rider) => rider.name),
    ['Rider B'],
  );
  assert.deepEqual(
    result.payload.groups.offline.map((rider) => rider.name),
    ['Rider C'],
  );
});
