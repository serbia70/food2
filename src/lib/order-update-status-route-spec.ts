...
test('forwardOrderUpdateStatus returns 400 invalid_json for malformed body', async (t) => {
  useTestEnv(t);
  const calls = useMockFetch(t, async () => jsonResponse({ success: true }));
  ...
  assert.deepEqual(body, { success: false, error: 'invalid_json' });
  assert.equal(calls.length, 0);
});

test('forwardOrderUpdateStatus prefers routeId over body id and rewrites payload id to route id', async (t) => {
  useTestEnv(t);
  const calls = useMockFetch(t, async () => jsonResponse({ success: true }));

  const response = await forwardOrderUpdateStatus(new Request('https://example.com/api/order/update_status/202', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  assert.equal(response.status, 200);
  assert.equal(calls.length, 1);
  assert.equal(calls[0]?.url, `${API_BASE_URL}/api/order/update_status/202`);
  const forwarded = JSON.parse(String(calls[0]?.body || '{}')) as { id?: unknown };
  assert.equal(forwarded.id, 202);
});

test('forwardOrderUpdateStatus forwards unknown fields unchanged instead of interpreting admin-only semantics', async (t) => {
  useTestEnv(t);
  const calls = useMockFetch(t, async () => jsonResponse({ success: true }));
  ...
  const response = await forwardOrderUpdateStatus(new Request('https://example.com/api/order/update_status', {
    ...
    body: JSON.stringify({
      ...
      status: 'picked_up',
      pickupEtaMinutes: 12,
      riderBroadcastedAt: '2026-04-15T10:00:00.000Z',
      customFlag: true,
    }),
  }));
  ...
  assert.equal(response.status, 200);
  assert.equal(calls.length, 1);
  const forwarded = JSON.parse(String(calls[0]?.body || '{}')) as Record<string, unknown>;
  assert.equal(forwarded.id, 101);
  assert.equal(forwarded.pickupEtaMinutes, 12);
  assert.equal(forwarded.riderBroadcastedAt, '2026-04-15T10:00:00.000Z');
  assert.equal(forwarded.customFlag, true);
});
