...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/api/master/trigger-backup`, {
      method: 'POST',
    ...
    return new Response(JSON.stringify({ success: false, error: e?.message || 'proxy failed' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
};
