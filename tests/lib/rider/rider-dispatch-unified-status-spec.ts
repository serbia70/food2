...
const riderDashboardSource = readFileSync(new URL('../../../src/pages/rider/dashboard.astro', import.meta.url), 'utf8');
...
test('rider-dispatch module still imports when Intl timezone formatter is unavailable', async () => {
  ...
  const moduleUrl = new URL(`../../../src/lib/rider-dispatch.ts?intl-fallback=${Date.now()}`, import.meta.url).href;
  try {
