...
import { mapTelegramClaimErrorToCallbackText } from '../../../src/pages/api/telegram/webhook.ts';
...
test('mapTelegramClaimErrorToCallbackText maps business errors to callback text', () => {
  ...
  assert.equal(mapTelegramClaimErrorToCallbackText({ error: 'dispatch_invalidated', reason: '其他原因' }), '操作失败');
});
